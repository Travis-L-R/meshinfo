...
class MemoryDataStore:
        ...
        if self.config['integrations']['geocoding']['enabled']:
            ...
            if 'latitude_i' in n['position'] and 'longitude_i' in n['position'] and n['position']['latitude_i'] is not None and n['position']['longitude_i'] is not None:
                if n['position']['geocoded'] is None or n['position']['last_geocoding'] is None or n['position']['last_geocoding'] < datetime.now().astimezone(ZoneInfo(self.config['server']['timezone'])) - timedelta(minutes=60):
                    try:
                        geocoded = utils.geocode_position(self.config['integrations']['geocoding']['geocode.maps.co']
                                                          ['api_key'], n['position']['latitude_i'] / 10000000, n['position']['longitude_i'] / 10000000)
                        if geocoded is not None:
                            n['position']['geocoded'] = geocoded
                            n['position']['last_geocoding'] = datetime.now().astimezone(
                                ZoneInfo(self.config['server']['timezone']))
                    except Exception as e:
                        logging.warning(f"Failed to geocode position: {e}")

        n['active'] = True
        if 'last_seen' in n and n['last_seen'] is not None and isinstance(n['last_seen'], str):
            n['last_seen'] = datetime.fromisoformat(n['last_seen']).astimezone(
                ZoneInfo(self.config['server']['timezone']))
        n['since'] = datetime.now().astimezone(
            ZoneInfo(self.config['server']['timezone'])) - n['last_seen']
        n['last_seen'] = datetime.now().astimezone(
            ZoneInfo(self.config['server']['timezone']))
        self.nodes[id] = n

    def load(self):
        try:
            nodes = self.load_json_file(
                f"{self.config['paths']['data']}/nodes.json")
            if nodes is not None:
                ...
                self.nodes = nodes
            logging.info(
                f"Loaded {len(self.nodes)} existing nodes from file ({self.config['paths']['data']}/nodes.json)")
        except FileNotFoundError:
            self.nodes = {}
        if self.config['server']['node_id'] not in self.nodes:
            self.nodes[self.config['server']['node_id']] = Node.default_node(
                self.config['server']['node_id'])
        self.nodes['ffffffff'] = Node.default_node('ffffffff')

        try:
            nodes_overrides: dict | None = self.load_json_file(
                f"{self.config['paths']['data']}/nodes-overrides.json")
            if nodes_overrides is not None:
                for id, node_override in nodes_overrides.items():
                    if id in self.nodes:
                        logging.info(f"Overriding node {id}")
                        node = self.nodes[id]
                        if 'position' in node_override:
                            logging.info("Overriding node position")
                            node['position'] = node_override['position']
                        self.nodes[id] = node
                logging.info(
                    f"Loaded {len(nodes_overrides.keys())} nodes overrides from file ({self.config['paths']['data']}/nodes-overrides.json)")
        except FileNotFoundError:
            pass

        try:
            chat = self.load_json_file(
                f"{self.config['paths']['data']}/chat.json")
            if chat is not None:
                self.chat = chat
            logging.info(
                f"Loaded {len(self.chat['channels']['0']['messages'])} chat messages from file ({self.config['paths']['data']}/chat.json)")
        except FileNotFoundError:
            self.chat = {
                'channels': {
                    '0': {
    ...
    async def save(self, immediately=False):
        ...
        if 'enrich' in self.config['server'] and self.config['server']['enrich']['enabled']:
            if since_last_backfill >= self.config['server']['enrich']['interval']:
                ...
                logging.info(
                    f"Enriched in {round(end.timestamp() - save_start.timestamp(), 2)} seconds")
        ...
        if immediately or since_last_data >= self.config['server']['intervals']['data_save']:
            ...
            logging.info(
                f"Saved json data in {round(end.timestamp() - save_start.timestamp(), 2)} seconds")
        ...
        if since_last_render >= self.config['server']['intervals']['render']:
            static_html_renderer = StaticHTMLRenderer(
                self.config, copy.deepcopy(self))
            await static_html_renderer.render()
            end = datetime.now(ZoneInfo(self.config['server']['timezone']))
            logging.info(
                f"Rendered in {round(end.timestamp() - save_start.timestamp(), 2)} seconds")
        ...
        if 'backups' in self.config['server'] and self.config['server']['backups']['enabled']:
            if since_last_backup >= self.config['server']['backups']['interval']:
                ...
                logging.info(
                    f"Backed up in {round(end.timestamp() - save_start.timestamp(), 2)} seconds")
    ...
    async def backup(self):
        ...
        logging.info(f"Backing up to {base_name}.tar.bz2")
        ...
        shutil.make_archive(
            ...
            base_dir=".",
            verbose=True)
        logging.info(f"Backed up to {base_name}.tar.bz2")
        ...
        if 'max_backups' in self.config['server']['backups'] and self.config['server']['backups']['max_backups'] > 0:
            ...
            logging.info(
                f"Deleting old backups (max {self.config['server']['backups']['max_backups']}, found {len(files)})")
    ...
    async def backfill_node_infos(self):
        ...
        logging.info(
            f"Nodes needing enrichment: {len(nodes_needing_enrichment)}")
    ...
    async def enrich_nodes(self, node_to_enrich):
        async with aiohttp.ClientSession() as session:
            ...
            logging.info(f"Enriching nodes: {','.join(node_ids)}")
            if self.config['server']['enrich']['provider'] == 'bayme':
                for node_id in node_ids:
                    logging.info(f"Enriching {node_id}")
                        ...
                        async with session.get(url) as response:
                            ...
                            if response.status == 200:
                                ...
                                for node_id, node_info in data.items():
                                    logging.info(f"Got info for {node_id}")
                                    if node_id in self.nodes:
                                        logging.info(f"Enriched {node_id}")
                                ...
                                logging.info(
                                    f"Failed to get info for {node_id}")
                    except Exception as e:
                        logging.warning(
                            f"Failed to get info for {node_id}\n{e}")
                ...
                try:
                    async with session.get(url) as response:
                        if response.status == 200:
                            data = await response.json()
                            for node_id, node_info in data.items():
                                logging.debug(f"Got info for {node_id}")
                                if node_id in self.nodes:
                                    logging.debug(f"Enriched {node_id}")
                                    node = self.nodes[node_id]
                                    node['shortname'] = node_info['shortName']
                                    node['longname'] = node_info['longName']
                                    self.nodes[node_id] = node
                        else:
                            logging.info(f"Failed to get info for {node_ids}")
                except Exception as e:
                    logging.warning(f"Failed to get info for {node_ids}\n{e}")
        ...
        if include_neighbors:
            ...
            if 'neighborinfo' in node and node['neighborinfo'] is not None and 'neighbors' in node['neighborinfo'] and len(node['neighborinfo']['neighbors']) > 0:
                for neighbor in node['neighborinfo']['neighbors']:
                    nn = self.find_node_by_hex_id(utils.convert_node_id_from_int_to_hex(
                        neighbor["node_id"]), include_neighbors=False)
                    if nn is not None:
            ...
            for nid, n in self.nodes.items():
                if 'neighborinfo' in n and n['neighborinfo'] is not None and 'neighbors' in n['neighborinfo'] and len(n['neighborinfo']['neighbors']) > 0:
                    if id in n['neighborinfo']['neighbors']:
                        nn = self.find_node_by_hex_id(
                            utils.convert_node_id_from_int_to_hex(nid), include_neighbors=False)
                        if nn is not None:
    ...
    def graph_node(self, node_id: str) -> dict | None:
        logging.debug(f"Graphing node: {node_id}")
        ...
        def recursive_graph_node(node_id, start_id="", level=0) -> dict | None:
            ...
            logging.debug("%s - %s" % ("  " * level, node_id))
            ...
            if node['neighborinfo'] and node['neighborinfo']['neighbors']:
                # logging.debug(f"Node {node_id} has {len(node['neighborinfo']['neighbors'])} neighbors")
                for neighbor in node['neighborinfo']['neighbors']:
                    nid = utils.convert_node_id_from_int_to_hex(
                        neighbor["node_id"])
                    if start_id is not None and start_id == nid or (self.config['server']['graph']['max_depth'] is not None and level >= self.config['server']['graph']['max_depth']):
                        continue
                    nn = recursive_graph_node(
                        nid, start_id=start_id, level=level+1)
                    if nn is not None:
                        neighbors_heard.append(nn.copy())
